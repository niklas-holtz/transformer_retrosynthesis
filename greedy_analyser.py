...
class GreedyAnalyser:
    ...
    def analyse(self, product, tokenizer, beam_size=6):
        ...
        iter = 1
        fail = False
        while not self.nodes_finished(nodes) and not fail:
            print("> Iteration: " + str(iter))

            new_nodes = []
            for node in nodes:
                for leave_idx, leave in enumerate(node.get_unfinished_leaves()):
                    token_decoded, all_token_scores, all_token_seq_decoded = self.translator.predict(leave.product, tokenizer, beam_size=beam_size, minimum_predictions=beam_size)
        ...
        child1 = ProductNode('dasdasdasdas')
        child1.finished = False
        product.add_reactant(child1)
        child2 = ProductNode('cascascas')
        product.add_reactant(child2)
        child3 = ProductNode('dascascas')
        child3.finished = False
        child2.add_reactant(child3)
        child4 = ProductNode('yxcyx')
        child2.add_reactant(child4)
        print(product)
        print(product.is_finished())

        p_copy = copy.deepcopy(product)
        p_copy.reactants[0].product = 'asdasddxyyyyyyy'

        print(product)
        print(p_copy)
    

        for leave in product.get_unfinished_leaves():
            print(leave)
        """

        return None

    def nodes_finished(self, nodes):
        for node in nodes:
            if node.is_finished():
                return True
        return False
...
class ProductNode:
    ...
    def __str__(self, level=0):
        ...
        ret = "\t" * level + repr(self.product) + ' (score = ' + str(self.score) + ') ' + finished_str(self.finished) + "\n"
        for child in self.reactants:
